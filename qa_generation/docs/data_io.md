...
flowchart TB
    ...
    subgraph Ext["外部"]
        CSV["チャンク済み CSV"]
        Cfg["config.DATASET_CONFIGS"]
        Out["output_dir（QAPipeline の既定 qa_output/pipeline）"]
    end
    CLI --> Pipe
    Web --> Pipe
...
| `services/__init__.py` | 旧 `dataset_service.py` を 2026-09-12 に削除した旨を記載。以後、データセット読み込みの経路は本モジュールだけである（`load_uploaded_file` / `load_preprocessed_data` の呼び出し元は `pipeline.py` のみ。grep 実測） |
