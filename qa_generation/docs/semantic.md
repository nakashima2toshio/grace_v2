...
| 機能 | 説明 |
|---|---|
| `SemanticCoverage(embedding_model=ModelConfig.EMBEDDING_MODEL)` | Embedding クライアントとトークナイザを用意する |
...
flowchart TB
    ...
    subgraph EXTERNAL["外部（LLM・Embedding・ファイル・基盤）"]
        ...
        TOK["トークン数: create_llm_client('anthropic').count_tokens（tiktoken 近似）"]
        MEC["MeCab（任意・文分割）"]
    end
    EV --> SC
    HR --> SC
