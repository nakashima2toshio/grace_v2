...
| **`qa_generation/models.py`（本モジュール）** | `qa_generation/__init__.py` が再エクスポートするだけで、**ほかに import 元が無い**（2026-09-24 に grep で実測） | `question` / `answer` / `question_type` / `difficulty` / `source_span` |
| `helper/helper_rag_qa.py` | 統合元として残っている旧定義 | 同じ 5 フィールドだが**すべて必須**（既定値なし） |
...
> 📌 **トップレベルはオブジェクトにする。** `AnthropicClient.generate_structured()` は
> 応答から最初の `{` と最後の `}` の間を切り出して `model_validate_json()` する。
> `response_schema` も `BaseModel` を受ける型なので、`List[QAPair]` を直接スキーマにはできない。
> `QAPairsList` のように `qa_pairs` キーで包んだ形を使う。
