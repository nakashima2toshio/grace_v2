...
| 3 | Q/A を生成する（逐次 / Celery 並列） | `generate_qa()` / `_generate_sync()` / `_generate_with_celery()` | `SmartQAGenerator`（Anthropic Claude API）を直接、または Celery ワーカー経由で呼ぶ |
...
flowchart TB
    ...
    subgraph EXTERNAL["外部（LLM・Embedding・ファイル・基盤）"]
        ...
        SG["SmartQAGenerator → Anthropic Claude API"]
...
3. `generate_qa()` が逐次なら `SmartQAGenerator`（Anthropic Claude API）、並列なら Celery ワーカーへ投入して Q/A を集める
