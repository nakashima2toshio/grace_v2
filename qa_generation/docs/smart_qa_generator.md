...
| 2 | LLM クライアントを用意する | `SmartQAGenerator.__init__()` | `create_llm_client("anthropic")` でクライアントを生成し、既定モデルを保持する |
...
flowchart TB
    ...
    subgraph EXTERNAL["外部（LLM・Embedding・ファイル・基盤）"]
        LLMC["helper.helper_llm.create_llm_client('anthropic')"]
...
2. `analyze_and_generate()` が Anthropic Claude API へ分析と生成を 1 回の構造化出力（`SmartQAResult`）で依頼する
