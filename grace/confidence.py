    ...
    def evaluate_with_factors(
        ...
        Args:
            query: ユーザーの質問。
                ...
                評価器が何を探していたのか分からないまま採点していた。実測
                （2026-08-17 11:10 / 11:41）では評価理由にこう書かれていた:

                    「具体的な検索クエリが不明確なため、結果が適切に
                      マッチしているか判断困難」
                    「元の質問が空文字列（''）であるため、検索意図が不明確」

                後者は評価器の作話だが、**質問が渡っていないという指摘自体は
                正しい**。信頼度は回答ゲートの判断材料なので、判定できない
                まま採点されるのは困る。
...
class SourceAgreementCalculator:
    ...
    def _embed_all(self, answers: List[str]) -> List[List[float]]:
        ...
        ## なぜ 1 件ずつではないのか（実測 2026-08-17 16:17）
        ...
        Embedding は外部 API（Gemini）なので、待ち時間だけでなく**課金にも効く**。
        `contents` はリストを受けられるので、内容も件数も変えずに 1 往復へ畳める。
