...
まとめる。各要素の IPO 詳細は既存の個別ドキュメント（`grace/doc/confidence.md` /
`grace/doc/calibration.md`）に委ね、本書はアーキテクチャ＋データフロー＋要点に徹する。
...
> IPO 詳細は `grace/doc/confidence.md` を参照。ここでは処理内容の要点のみ。
...
> IPO 詳細は `grace/doc/calibration.md` を参照。
...
| ドキュメント | 内容 |
|---|---|
| `grace/doc/confidence.md` | `confidence.py` の IPO 詳細（各クラス/関数） |
| `grace/doc/calibration.md` | `calibration.py` の IPO 詳細 |
| `grace/doc/executor.md` | `_calculate_overall_confidence` を含む実行エンジン |
