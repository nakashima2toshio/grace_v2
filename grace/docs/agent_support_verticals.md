...
**実装状況**: `VerticalProfile` 導入と gov/saas/ec の 3 プロファイルは実装済み（PR #106）。設計時の実装順（自治体 → SaaS → EC）どおり 3 業界を同時に組み込み済みで、上表のとおり全項目が配線済み。残件は §9 を参照。
...
（登録は `qa_qdrant/register_to_qdrant.py`。プロファイルのコレクションが未登録なら警告のうえ無制限検索になる）。
...
| 2.0 | **`eval/vertical/` の記述を削除**（2026-09-04）。KPI 評価基盤 `eval/vertical/` は `grace_v2` にも `grace_v2_local` にも存在せず（`git log --all --full-history -- 'eval/*'` が両方とも空）、他プロジェクト由来の記述だった。旧 §8「テスト用データ」・旧 §9.1「KPI 評価」を章ごと削除し、ヘッダーの「gov 7/7・saas 8/8・ec 9/9＝decision_accuracy 1.000」という**存在しない基盤での計測値**も撤去。§9→§8・§10→§9・§11→§10 へ採番し直し。残タスク #3（KPI 評価スクリプト）/ #8（テスト用コレクション）/ #13（実運用ナレッジ取得）を「✅ 実装済み」から **「❌ 本リポジトリには無い」** へ訂正。テスト表を実在する `backend/tests/` の 3 件へ差し替え（リポジトリ直下に `tests/` は無い）。金額の実測値も、存在しない基盤での計測値のため撤去 |
