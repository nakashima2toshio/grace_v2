...
class ReasoningTool(BaseTool):
    ...
    # 参照情報の見出しに出す種別。回答規則 3 がこの語をそのまま使う。
    _ORIGIN_INTERNAL = "社内"
    _ORIGIN_WEB = "Web"
    ...
    def _source_origin(cls, source: Dict) -> str:
        ...
        でも判定する（`gates._collect_citations` と同じ規則）。
        """
    ...
    def _build_prompt(
        ...
        # ⚠️ **現在日時を渡す。** LLM は今日が何日かを知らないため、日付が無いと
        # 相対表現（明日・今週・先月）を解決できず、参照情報に答えがあっても
        # 取り出せない。
        #
        # 実測（「明日の東京の天気は？」）: 情報源に予報があったのに
        ...
        if sources:
            ...
            for i, source in enumerate(sources, 1):
                ...
                #    「社内ナレッジによると」と書かれる**（_source_origin 参照）。
