...
それぞれの doc は **2026-09-19 に削除した**。これらは CLI
...
| バージョン | 変更内容 |
|-----------|---------|
| 2.0 | S0〜S9 のトレース用スタブと doc を削除（CLI `agent_support_example.py` の削除にともない呼び出し先を喪失）。索引を `benchmark.py` のみへ縮約（2026-09-19） |
