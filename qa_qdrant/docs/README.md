...
| 文書 | 内容 | 行数 | Ver | 重要度 |
|---|---|---:|---|:--:|
| [`01_install.md`](01_install.md) | 環境構築。MeCab・Docker（Qdrant / Redis）・Celery・API キー。**運用の唯一の入口** | 861 | 2.5 | ★★★ |
...
|---|---|---:|---:|---|:--:|
| [`register_to_qdrant.md`](register_to_qdrant.md) | `register_to_qdrant.py` — 既存 CSV → Qdrant | 588 | 588 | 2.3 | ★★★ |
| [`make_qa.md`](make_qa.md) | `make_qa.py` — Q/A 生成のみの CLI | 265 | 450 | 3.5 | ★★☆ |
| [`qdrant_delete_collection.md`](qdrant_delete_collection.md) | **`qdrant_delete_collection.py`（リポジトリ直下）** — コレクション削除 CLI | 73 | 304 | 1.1 | ★★☆ |
| [`make_qa_register_qdrant_ipo.md`](make_qa_register_qdrant_ipo.md) | `make_qa_register_qdrant.py` — Q/A 生成 → Qdrant 登録の統合 CLI。§3.3 の既知の問題 5 件は修正済み | 708 | 701 | 1.7 | ★★★ |
| [`make_qa_qapipeline.md`](make_qa_qapipeline.md) | `QAPipeline` ＋ `SmartQAGenerator` の連携（**実体は `qa_generation/`**） | — | 938 | 1.4 | ★★☆ |
...
| 文書 | 内容 | 行数 | Ver | 重要度 |
|---|---|---:|---|:--:|
| [`qa_qdrant_architecture.md`](qa_qdrant_architecture.md) | Q/A 生成 & Qdrant 登録システムの設計書（v3.0） | 799 | 3.4 | ★★☆ |
