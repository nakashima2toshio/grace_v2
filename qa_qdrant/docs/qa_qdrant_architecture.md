...
| # | 責務 | 対応モジュール | 説明 |
|---|---|---|---|
| 1 | チャンク済み CSV から Q/A を生成する | `qa_generation/pipeline.py`（`QAPipeline`） | `qa_generation/smart_qa_generator.py` が Anthropic Claude で生成 |
...
2. `SmartQAGenerator` が Anthropic Claude で Q/A を生成する（`--use-celery` なら Celery ワーカーで並列）
