...
> ## 📌 本書に出てくる `gpt-4o` / `gemini-2.0-flash` / `text-embedding-3-*` について
>
> 本モジュールは**トークン計算・コスト概算の互換テーブル**であり、
> `token_service.py:34-93` に実在する定義をそのまま記述している。
> **表の内容は正しいので変更しない。**
>
> ただし、**このプロジェクトが実際に使うモデルは次のとおり**である（CLAUDE.md §3）。
>
> | 用途 | プロバイダ | 既定 |
> |---|---|---|
...
Embeddingモデル価格表（$/1000トークン）。**表そのものは `config.py::ModelConfig.EMBEDDING_PRICING`**
で、本モジュールはそれを参照するだけ（`helper_embedding` / `helper_llm` も同じ表を指す）。
