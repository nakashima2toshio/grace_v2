...
`qa_generator_runner` を import する死にコードだったため 2026-09-12 に削除した。
"""
...
def generate_qa_pairs(
    ...
    """
    テキストからQ/Aペアを生成（Anthropic Claude を使用）

    Args:
        text: 対象テキスト
        dataset_type: データセットタイプ
        chunk_id: チャンクID
        model: 使用するモデル（デフォルト: claude-sonnet-5）
...
"""
    ...
    try:
        # 構造化出力 API を使用（Anthropic Claude）
