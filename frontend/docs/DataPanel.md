...
| 子 | `DataJobPanel.tsx`（チャンキング / Q/A 生成 / 登録）、`CollectionPanel.tsx`（コレクション管理） |
| 主な依存 | `./CollectionPanel`, `./DataJobPanel` |
| 対応バックエンド | `backend/app/api/data.py`, `backend/app/api/qdrant.py` |

### 主な責務

- データ準備の 3 工程（＋コレクション管理）を**パイプラインの流れ順**にサブタブとして並べる。
...
`DataJobPanel` はチャンキング・Q/A 生成・登録で**同じコンポーネント**なので、
...
| テストファイル | 対象 | 実行 |
|---|---|---|
| `src/state/dataReducer.test.ts` | 子が使う reducer（24 ケース） | `npm test` |
| `src/state/dataParams.test.ts` | 子が使うフォーム純関数（34 ケース） | `npm test` |
