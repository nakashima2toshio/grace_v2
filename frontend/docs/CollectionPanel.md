...
| エラーが支援技術に伝わるか | ✅ `.error-banner` 2 件と Qdrant 停止の `.warn-banner` に `role="alert"` |
| 削除が実行されなかったことが伝わるか | ✅ 中止の `.warn-banner` に **`role="status"`**（v1.3 で追加）。利用者の拒否かタイムアウトによる**非破壊の結果**なので、割り込む `alert` ではなく polite な `status` を使う |
...
| 版 | 日付 | 変更内容 |
|---|---|---|
| 1.3 | 2026-09-12 | **中止バナーに `role="status"` を追加。** 「削除は実行されませんでした」だけ `role` が無く、支援技術に伝わっていなかった。利用者の拒否／承認待ちのタイムアウトはどちらも**非破壊で安全側に倒れた結果**なので、割り込む `alert` ではなく polite な `status` を選んだ。あわせて「エラーが支援技術に伝わるか」の記述を実態（どのバナーに何が付いているか）へ精緻化 |
| 1.2 | 2026-09-12 | **実装と突き合わせて差分が無いことを確認**（Props の TS ブロック・ステップ数・export シグネチャ）。内容の修正は不要で、遅れていたのはヘッダーの日付だけだった。検証した事実を残すため版を上げる |
| 1.0 | 2026-08-05 | 初版作成 |
