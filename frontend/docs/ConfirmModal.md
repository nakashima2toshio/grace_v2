...
> 📌 2026-09-24 にフォーカストラップを入れたため、`useRef`（`modalRef`）と
...
| モーダルにフォーカストラップがあるか | ✅ **実装済み**（2026-09-24）。Tab / Shift+Tab は端で巻き戻り、背面へ抜けない。移動先の計算は `state/focusTrap.ts::nextFocusIndex` |
| 開いたときにモーダル内へフォーカスが移るか | ✅ **実装済み**（2026-09-24）。マウント時に最初の要素（承認ボタン）へ `focus()` |
...
> 📌 フォーカス関連で残る ❌ 2 件は**いずれも判断のうえで未対応**（`Escape` は意図的、
> フォーカス復帰は戻し先が自明でない）であり、単なる実装漏れではない。
...
| `src/state/focusTrap.test.ts` | フォーカストラップの移動先計算 `isTabKey` / `nextFocusIndex`（**12 ケース**） | `npm test` |
| （コンポーネント本体の専用テストなし） | — | — |
...
| 1.2 | 2026-09-24 | **フォーカストラップと初期フォーカスを実装**（grace_v2_local から移植）。Tab / Shift+Tab が端で巻き戻るようにし、マウント時に承認ボタンへ焦点を移す。移動先の計算は `state/focusTrap.ts` の純関数（`isTabKey` / `nextFocusIndex`・**12 ケース**）。§8 の ❌ のうち 2 件が ✅ になり、残る 2 件（`Escape`・フォーカス復帰）は判断のうえで未対応であることを明記した |
