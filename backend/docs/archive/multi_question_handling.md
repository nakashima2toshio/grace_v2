...
> 🗄️ **アーカイブ（2026-09-15）**: 本書の**確定仕様（§0・§13）は
> [`../support_flow.md` §5](../support_flow.md#6-0-a-複数質問クエリと担当範囲外) へ統合済み**。
> 本書は**採用しなかった案（§1〜§12 の fan-out 系ロードマップ）の記録**として残す。
> 実装の正を知りたい場合は `support_flow.md` §6 を見ること。


**Version 3.0** | 最終更新: 2026-08-29 | ステータス: **実装済み（パイプラインへ組み込み済み）**

> ✅ **v3.0 で実装が完了した。** §0 が実装の正で、§1 以降は**採用しなかった案の記録**
> および設計の背景として残す。採用したのは §2 の 3 案（A/B/C＝N 問すべてに答える系）
> ではなく、**§13 の絞り込み方式**である。確定した仕様は次の 2 点。
>
...
> 📌 §1〜§12 は実装前に書かれた分析である。**現在の実装は §0 を参照すること。**
>
...
2. [問題の所在（コード事実）](#1-問題の所在コード事実)
   ...
   - [構造的ブロッカー: グローバル可変 config（解消済み）](#12-構造的ブロッカー-グローバル可変-config--解消済みv12)
