...
> groundedness が条文からほぼ必ず「支持される」と判定してしまう。姉妹リポジトリの
...
文書分割は **LLM を使わない決定的な処理**で、原文オフセットを保持する（`split_segments()`）。
行の種別はこの 3 つの正規表現だけで決まる。

| 定数 | パターン | 役割 |
|------|----------|------|
| `_LIST_RE` | `^\s*(?:[・\-*＊]\|[0-9０-９]+[.)．）])` | 箇条書きの行頭（`・` `-` `*` `＊` と、半角/全角の数字＋`.` `)` `．` `）`）。**1 行 1 セグメント**にする |
| `_HEADING_RE` | `^\s*(?:#{1,6}\s\|[■◆●▼【])` | 見出しの行頭（Markdown の `#`〜`######` と `■ ◆ ● ▼ 【`）。同じく 1 行 1 セグメント |
| `_SENTENCE_END_RE` | `[。！？!?]` | 日本語・英語の文末。`MAX_SEGMENT_CHARS`（400）を超える段落をここで再分割する |

> 📝 全角文字を明示的に含めている点に注意（`＊` / `０-９` / `．` / `）` / `！？`）。
> 日本語の文書は全角で書かれることが多く、半角だけでは箇条書きも文末も取りこぼす。
