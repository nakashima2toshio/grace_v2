...
|------|------|
| **Input** | なし |
| **Process** | `get_config().llm` から `model` / `light_model` / `heavy_model` を取り、データ準備側の既定は `ChunkingRequest` / `QaGenerationRequest` の**スキーマ既定値**から、Embedding は `ModelConfig.EMBEDDING_MODEL` / `EMBEDDING_DIMS` から引く |
| **Output** | `ModelInfo` |
