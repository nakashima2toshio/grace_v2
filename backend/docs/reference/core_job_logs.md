...
| # | 責務 | 対応モジュール | 説明 |
|---|------|--------------|------|
| 1 | 取り付け・取り外し（とステップの切り替え） | `backend/app/core/job_logs.py` | `capture_logs()` が `JobLogHandler` を取り付け、終了時に必ず外す。転送は `JobLogHandler.emit()`、ステップの切り替えは `set_step()` |
...
| バージョン | 変更内容 |
|-----------|---------|
| 1.3 | 概要の「各責務対応の要素」（4 行・主な責務 3 項目と非対応）を、基本フォーマットの「各責務対応のモジュール」（主な責務と 1:1 の 3 行）へ置き換えた（2026-09-24） |
