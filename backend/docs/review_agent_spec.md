...
| 1.3 | **§5.3 のルール一覧からキーワード表を削除し、正本（`rulesets.py` / `core_rulesets.md` §5.4）へのリンクに置換**（2026-09-15）。21 行の表で `keywords` まで複製しており、**実測すると腐っていた**——キーワードを持つ 15 ルール**全部**で語が欠落（`yakki-02` は実装 15 語に対し文書 4 語、`keihyo-11` は 6 語に対し 4 語）、さらに `yakki-04`（安全性の保証表現）は**文書に一度も出てこなかった**。見出しも「21 件」「医薬品医療機器等法（3 件）」のままだった。同じ表を 2 箇所に持つ限り必ず腐るので、本書は**法令別の件数と判定方式の要約**だけを持つ形に改めた（実測 2026-09-15: 景表法 12 / 薬機法 4 / 特商法 6 / 社内規程 1 ＝ 23、`always_check` 7 / `web_check` 5）。`policy-01` の実測記録と特商法が `always_check` である理由は残した。あわせて **v1.2 でヘッダーの版数を上げ忘れていた**（本文 1.1 / 変更履歴 1.2）のを是正 |
