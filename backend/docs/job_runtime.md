...
> 系統別文書（`support_flow.md` / `review_flow.md` / `data_pipeline.md`）は
> **本書を参照するだけ**にして、同じ説明を持たない。
...
> - [`reference/core_job_logs.md`](./reference/core_job_logs.md)
...
エージェントの 1 周は数十秒〜数分かかるため、HTTP リクエストの中で完結させない。
`POST` は **202 を即返し**、実処理はワーカースレッドで走り、進捗は SSE で配信する。
HITL 承認だけは処理の途中で人の応答が要るので、**SSE で問い、別の POST で答える**。
...
（API キー未設定などの「想定内の失敗」を runner が自分で説明するため）。
...
| `log` | 途中経過メッセージ（CLI の print 相当） | 各 runner・`job_logs` |
...
> 組み立てる別物である（[§ api_contract.md](./api_contract.md)）。
> `ts`（完了時刻）と `started_at`（**POST を受け付けた時刻** = `Job.created_at`）を
> 必ず載せる。フロントの所要時間表示（`frontend/src/state/elapsed.ts`）が
> サーバ時計からこの 2 つを取るため、欠けると「完了 … ／ 所要 …」の行が消える。

---

## 3. runner 注入（3 系統を 1 基盤に載せる仕掛け）

当初 `jobs.py` は `run_support_agent_core` を直接呼んでいた。Review とデータ準備を
同じ基盤へ載せるにあたり、**実行関数を差し替え可能**にしてある。
...
実行する。一見すると「import 忘れ = 登録漏れ」に見えるが、**構造的に起きない**。
`ReviewParams` や `ChunkingParams` を構築するには、そのモジュールを import する
必要があるためである。

この形にすることで **`jobs.py` は Review・データ準備を一切知らずに済み**、
循環 import も発生しない（[`architecture.md` §5](./architecture.md)）。
...
一方 Web では、応答は**別の HTTP リクエスト**として非同期に届く。この段差を
`threading.Event` で埋めるのが `InterventionBridge` である。
...
→ `DEFAULT_CONFIRM_TIMEOUT`（300 秒）**。時間切れになったら

```python
return InterventionResponse(action=InterventionAction.CANCEL, timeout_reached=True)
```

を返す。**＝実行しない。** 呼び出し側（`_perform_action`）はこれを見て有人対応へ倒す。
「返事が無い＝承認」には**絶対にしない**。

> ⚠️ **CLI の自動承認（`AUTO_PROCEED`）を Web に持ち込まないこと。**
> `support_agent.py` は CLI 用に `AUTO_PROCEED` を持つが、Web 経路は必ず
> `InterventionBridge` を通す（受け入れ条件 §5-2）。

### 同時に待てるのは 1 件だけ

`InterventionBridge._pending` は `Optional[PendingIntervention]` の**単数**である。
1 ジョブ内で 2 つの承認を並行して待つ設計にはなっていない。
`resolve()` は `intervention_id` が一致しなければ `False` を返し、API は
`not_waiting` を応答する（404 ではない）。

`selected_option` は選択肢つきの介入（0-(A) 主質問の選択）でのみ使い、
既定 `None` なので通常の CONFIRM 承認には影響しない。
...
`capture_logs` はコンテキストマネージャなので、例外時もハンドラは外れる。
...
| 失敗の種類 | どう見えるか |
|---|---|
| runner が例外を送出 | `error` イベント（`❌ 実行に失敗しました: <型>: <内容>` + Qdrant / API キーの hint）→ `failed` |
| runner が `None` を返す | runner が出した `error` イベントのみ → `failed` |
...
フロントは `done` を見て `EventSource` を閉じる。
...
| **シングルプロセス前提** | `job_manager` はモジュールシングルトン。uvicorn を `--workers 2` 以上で起動すると、別プロセスのジョブは見えない |
| **完了ジョブは 50 件まで**（`MAX_FINISHED_JOBS`） | 超えると古い完了ジョブから破棄され、結果取得が 404 になる |
| **認証なし** | CORS は `localhost:5173` / `127.0.0.1:5173` のみ許可 |
| **キャンセル API が無い** | 走り出したジョブを外から止める口は無い（daemon スレッドなのでプロセス終了では道連れになる） |
...
- [ ] 外部パッケージの進捗を出したい → `capture_logs()` を使ったか（`with` で必ず外れるか）
- [ ] `done_event` の `ts` / `started_at` を落としていないか（フロントの所要時間が消える）
- [ ] 3 系統すべてに効く変更か？ `backend/tests/test_jobs_generic.py` と Review 系
      18 ファイルを流したか
