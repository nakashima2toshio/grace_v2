...
> CLI 本体は 2026-09-19 に削除した（機能確認用の薄いラッパだったため）。
...
def run_support_agent_core(
    ...
    # UI（ヘッダーのモデルセレクタ）からの上書き。指定が無ければ設定の既定値。
    #
    ...
    # （実測 2026-08-30: 5.6 秒 + 7.0 秒。claim の分解結果 8 件も完全に一致で、
    # 2 回目に新しい情報は何も得られていない。起動ログにも
    # `GroundednessVerifier initialized` が 2 行出ていた）。
    #
    ...
    # 以前は 2 回呼んでおり、実測 2026-08-30（ローカル LLM）で 16.3 秒 ＋ 2.2 秒
    # かかっていた（前者はモデルのウォームアップ込み）。往復を 1 回に畳む。
    analysis = (
    ...
    if clusters and adopted_cluster_index is not None:
        ...
        # 結果はどちらも skipped で見分けがつかない。実測 2026-08-29 で第 2 段が
        # 単一と判断した際、ログが 1 行も無くて切り分けできなかった。
        skip_reason = (
    ...
    # 実測 2026-08-29（同一の注入）: Anthropic は断ったが、姉妹リポジトリの
    # ローカル LLM は住民票にだけ答えて天気に触れなかった。
    ...
    if not answer_cites_sources(support.answer, support.citations):
        ...
        # 出典セクションは別に出るので実害は小さいが、揺れが見えないと
        # プロンプトの効き目を評価できない。
