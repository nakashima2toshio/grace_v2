...
2026-09-19 に削除済みで、現在の参照元は `support_agent.py` / `review_agent.py`。
"""
...
def create_no_info_judge(
    ...
    実測 2026-08-17 16:17: 「明日の東京の天気は？」に対し天気・最高 30℃・
...
# =============================================================================
# 複数質問クエリ（`backend/docs/support_flow.md` §6）
# =============================================================================
#
...
# （`backend/docs/support_flow.md` §6.2・§6.6）。
...
def reconstruct_query(
    ...
    **planner・executor・gates の判定ロジックは一切改変しない。**
    ...
    # config が無ければ LLM を呼べない。素朴な連結へフォールバックする。
    if config is None:
    ...
    try:
        ...
        response = client.models.generate_content(
            ...
            contents=prompt,
            config={"temperature": 0.0, "max_output_tokens": 300},
        )
...
def deferred_main_questions(
    ...
    （`backend/docs/support_flow.md` §概要）と区別がつかなくなる。
...
def _append_missing_links(answer: str, links: Optional[Dict[str, str]]) -> str:
    ...
    （実測 2026-08-31: 回答本文に「ご注意（担当範囲外のお問い合わせについて）」
    と気象庁・e-Gov の URL が出た）。
...
def ensure_out_of_scope_notice(
    ...
    落ちる理由は、回答生成プロンプトの【回答の構成ルール】1（参照情報にある
    事実のみ）・7（捏造禁止）と衝突して見えるためと考えられる。プロンプト側でも
    例外だと明示したが、それでも従う保証はない。
