...
def run_review_agent_core(
    ...
    def _evaluate(rule, target: Segment, citations, source_texts) -> None:
        ...
        # 「たしかにそのルールに該当しうる」と支持してしまう（定型文はルール名を
        # 言い換えただけなので、条文からはほぼ必ず読み取れる）。結果、
        # **「自動判定に失敗したため要確認」と書かれた指摘が「確定」バッジ**で
        # 表示される（姉妹リポジトリ grace_v2_local の実測 2026-08-31: 適正な LP
        # でも 9 件中 8 件が確定と出た）。読む人は確定を信じるので、これは
