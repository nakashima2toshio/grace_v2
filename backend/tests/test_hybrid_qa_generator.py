...
`helper/helper_rag_qa.py` は先頭で `import spacy` する。spaCy が入っていない環境
（grace_v2 の CI は `requirements-test.txt` に spaCy を含まない）では空のモジュールを
差し込んで import する。`spacy.load` は呼ばない（`RuleBasedQAGenerator.__init__` を通さない）ので、
