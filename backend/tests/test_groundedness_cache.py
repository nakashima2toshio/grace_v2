...
「住民票の写しの取り方は？」1 リクエストのログ（2026-08-30）:

    04:07:41→04:07:47  executor._blend_groundedness_confidence  5.6 秒
    04:07:47→04:07:54  support_agent ③ 根拠評価                 7.0 秒

入力（query / answer / 5 件のソース）は完全に同一で、claim の分解結果も
8 件すべて一致していた。温度 0 なので 2 回目に新しい情報は無く、
**7.0 秒＝リクエスト全体 36 秒の 19%** がまるごと待ち時間だった。
起動ログにも `GroundednessVerifier initialized` が 2 行出ており、
別インスタンスが 2 つ立っていたのが原因。
...
class TestFailuresAreNotCached:
    ...
    def test_empty_response_is_retried(self):
        """空応答（本文 0 文字）も同じ扱い。入力ではなく実行時の事情。"""
        verifier, client = _verifier(supported=6, total=6)
        empty = MagicMock()
        empty.text = ""
        client.models.generate_content.side_effect = [
        ...
        assert verifier.verify(QUERY, ANSWER, SOURCES).verified is False
