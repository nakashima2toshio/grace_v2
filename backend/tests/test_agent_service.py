...
# `services/agent_service.py::ReActAgent`（Legacy ReAct 経路）の単体テスト。
# LLM は `create_llm_client` をモックし、`generate_with_tools` が返す `ToolUseResponse` を
# 手で組み立てる。実 API キー・Qdrant 不要。
...
def mock_create_client():
    """services.agent_service.create_llm_client をモックする（呼び出し引数を検査できる）。"""
    with patch("services.agent_service.create_llm_client", return_value=MagicMock()) as mock_create:
        yield mock_create


@pytest.fixture
def mock_llm(mock_create_client):
    """生成された LLM クライアント。generate_with_tools の戻り値を制御する。"""
    return mock_create_client.return_value
...
class TestReActAgent:
    ...
    def test_init(self, mock_llm):
        ...
        agent = ReActAgent(selected_collections=["coll1"], model_name="claude-sonnet-5")
    ...
    def test_init_uses_anthropic_client(self, mock_create_client):
        """LLM クライアントは本リポジトリのプロバイダ（Anthropic）で、指定モデルを既定に作る。"""
        ReActAgent(selected_collections=[], model_name="claude-sonnet-5")
        mock_create_client.assert_called_once_with("anthropic", default_model="claude-sonnet-5")

    def test_execute_turn_simple_answer(self, mock_llm):
        """モデルが直接回答を返すケース（ツール呼び出しなし）"""
        agent = ReActAgent(selected_collections=[], model_name="claude-sonnet-5")
