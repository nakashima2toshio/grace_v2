...
2026-09-25 まで、`qa_qdrant/make_qa_register_qdrant.py` は `--text-column` を入口の判定にだけ使い、
`QAPipeline` へ渡していなかった。`QAPipeline._load_chunks_from_csv()` は
`text` → `Combined_Text` → `content` → `chunk_text` の固定順で列を探すため、

- `--text-column body` で `body` 列しか無い CSV → 判定は通るのに生成で ValueError
- `body` と `text` の両方がある CSV → 指定を無視して `text` 列を使う

となっていた（経緯は `qa_qdrant/docs/make_qa_register_qdrant_ipo.md` §3.3 の 4）。
...
class _FakePipeline:
    ...
    def run(self, **kwargs):
        ...
        qa_csv = out / "qa_pairs_chunks_20260925_101500.csv"
