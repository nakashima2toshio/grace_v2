...
2026-09-25 まで、`--input-file *.txt` は `QAPipeline` へそのまま渡され、
...
チャンク化・Q/A 生成・Qdrant 登録はすべて差し替えるので、実 LLM / Qdrant / API キーは不要。
"""
...
class _FakePipeline:
    ...
    def __init__(self, dataset_name=None, input_file=None, model=None, output_dir=None, max_docs=None):
    ...
    def run(self, **kwargs):
        ...
        qa_csv = out / "qa_pairs_doc_chunks_20260925_101500.csv"
...
def env(monkeypatch, tmp_path):
    """チャンク化・Q/A 生成・登録を差し替え、呼び出しを記録する。"""
    import services.data_pipeline_service as dps

    calls = {"chunking": [], "registration": []}
...
def test_txt_is_chunked_then_passed_to_qa_generation(monkeypatch, tmp_path, env):
    ...
    _argv(monkeypatch, tmp_path, doc, "--chunk-model", "claude-haiku-4-5")
    ...
    assert chunk_call["model"] == "claude-haiku-4-5"
    ...
    assert env["registration"][0]["csv_path"].endswith("qa_pairs_doc_chunks_20260925_101500.csv")
...
def test_txt_without_anthropic_key_exits_with_1(monkeypatch, tmp_path, env):
    """チャンク化に要る `ANTHROPIC_API_KEY` が無ければ、チャンク化前に終了コード 1 で止まること。"""
