...
> 📝 **検出されても自動的に違反とは限らない。** `frontend/docs/` の各コンポーネント文書と
> `grace/step_trace/docs/` の各ステップ文書は、**定型の枠（4〜5 行）を各ファイルが持つ設計**で、
> 2026-09-15 時点の検出はこの 2 種のみ（実測）。§3.1 の 4 本どうし、または横断文書と
> 領域別 docs のあいだで出たものが**本当の違反**である。
