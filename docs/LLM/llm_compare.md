...
1. [比較の前提](#1-比較の前提)
2. [最重要の発見 qwen は成果物を出していない](#2-最重要の発見-qwen-は成果物を出していない)
...
| 5 | qwen3.5:9b | ローカル（Ollama・6.6 GB） | `react_ollama_qwen3_5_9b.md` | 224 | 39,196 |
...
- §2.3 の観察は**参考情報**として扱い、スコアには算入しない
...
| モデル | 欠陥 | 影響 |
|---|---|---|
| **qwen3.5:9b** | **そもそも文書になっていない。** 英語の内部推敲がそのまま保存されており、日本語比率 0.5% | **成果物として使えない** |
...
ただし **qwen3.5:9b の推敲は設問コードを誤って引用している**（§2.4）。
...
| 体裁の健全性 | ○ | ○ | ○ | ○ | **× 壊れる** | △ | **× 文書でない** |
| 分量の適切さ | **◎** | × 過大 | ◎ | ○ | △ 過少 | ○ | × 未完 |
| 事実誤認 | なし | なし | なし | なし | なし | なし | 引用の誤り（§2.4） |
...
| 7 | qwen3.5:9b | **解説ドキュメントを出力していない**（内部推敲のみで終了）。成果物が無いため最下位 |
