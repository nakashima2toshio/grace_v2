...
| 5 | L0 単発のモデル呼び出し | `helper/helper_llm.py` / `grace/llm_compat.py` / `helper/helper_embedding.py` | `create_llm_client` / `create_chat_client` / Gemini Embedding（§3） |
...
ツール呼び出し数・信頼度・経路一致）を CSV へ記録する計測ツールで、
L0〜L4 のいずれにも配置しない。

> 📝 同パッケージにあった `s0_arg.py`〜`s9_render.py`（S0〜S9 のステップ別トレース）は
