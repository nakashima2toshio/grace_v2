...
**Version 1.2** | 最終更新: 2026-08-25 | ステータス: **提案（一部は前提解消済み）**
...
## 13. 検討中の代替案: 絞り込み方式（未決定）

本書 §2 の 3 案（A/B/C）はいずれも「**N 問すべてに答える**」ことを前提にしている。
これとは別に、「**主質問を 1 つに絞り、採用しなかった質問を明示する**」方式が検討されている。

### 方式の概要

```
入力 → ① 複数質問検知 → ② 構造解析（主質問 / 関連質問）
     → ③ 構造を表示 → ④ クラスタを 1 つ採用 → ⑤ 通常パイプライン 1 周
     → ⑥ 回答 ＋ 保留した主質問を明示
```

**採用単位は「主質問」ではなく「主質問 ＋ その配下の関連質問」のクラスタ**とする。
「住民票の取り方は？ **その手数料は？**」のような従属質問は、切り離すと主質問の回答自体が
不完全になるため、**同一クエリとして 1 周で答える**。
...
| パイプライン実行回数 | N 周 | **1 周** |
| レイテンシ | N 倍 | 変わらない |
| ⑥ Action 二重発火（#17） | 対策必須 | **原理的に発生しない** |
| サブ質問ごとの vertical 解決（#19） | 必要 | **不要** |
| fan-out 検索（#10–#12） | 必要 | **不要** |
| 実装規模 | 23 項目 | **6 項目程度** |
| 回答の網羅性 | 全問に回答 | 1 クラスタのみ（残りは次回） |

### 成立の必須条件

> 🔴 **採用しなかった主質問を、成果物として明示的に返すこと。**
> これを怠ると、本書 §概要が「最も危険」とした事故（片方が無言で落ち、しかも
> `support_rate` が高いため高信頼として提示される）と**区別がつかなくなる**。
> 現行 `SupportResultModel` にはこの器が無く、**スキーマ追加が最小の必須作業**である。

### 未決定事項

| 論点 | 選択肢 |
|---|---|
| 選定主体 | A. ユーザーが選ぶ（対話・`ask_user`）／B. 自動選定＋事後表示／C. B ＋ 変更ボタン |
| 自動選定のルール | 「**最初に出現した主質問**」が既定案（説明可能性が高い）。LLM に重要度を判定させると選定理由を説明できなくなる |
| 「主質問 / 関連質問」の判定基準 | §1.3 の「独立／従属」と揃えるか、別基準にするか |
| 検知失敗時の倒し方 | **「単一とみなす」**（＝現行動作維持）。既存の `_detect_no_info_answer` 等が escalate へ倒すのとは**向きが逆**である点に注意 |

方針が決まり次第、本書を v2.0 として改稿する。
