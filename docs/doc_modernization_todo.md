...
**Version 2.6** | 作成日: 2026-09-12 | 最終更新: 2026-09-26 | 対象コミット: `d832a54`（master）
...
> **§10 の残タスクもすべて完了した**（最後のスクリーンショット 10 枚は 2026-09-26 に撮影）。
...
- [0. 現在地（実測サマリ）](#0-現在地実測サマリ)
- [1. P0 — CLAUDE.md の誤りを直す（他の全作業の前提）](#1-p0--claudemd-の誤りを直す他の全作業の前提)
- [2. ~~P0 — README.md のリンク切れ~~ → ⚠️ **v1.0 の指摘は誤りだった（訂正）**](#2-p0--readmemd-のリンク切れ---v10-の指摘は誤りだった訂正)
- [3. P1 — README_DATA.md の扱いを決める（重複 2,313 行）](#3-p1--readme_datamd-の扱いを決める重複-2313-行)
- [4. P1 — frontend/docs の欠落・陳腐化（テスト対象そのもの）](#4-p1--frontenddocs-の欠落陳腐化テスト対象そのもの)
- [5. P2 — Streamlit 残骸の除去（旧アプリからの移植漏れ）](#5-p2--streamlit-残骸の除去旧アプリからの移植漏れ)
- [6. P2 — LLM / モデル表記の仕分け](#6-p2--llm--モデル表記の仕分け)
- [7. P3 — 索引・重複の整理](#7-p3--索引重複の整理)
- [8. 確認事項（回答済み・2026-09-12）](#8-確認事項回答済み2026-09-12)
- [9. 実行順序（実施済み）](#9-実行順序実施済み)
- [10. 完了条件（実績）](#10-完了条件実績)
- [11. 変更履歴](#11-変更履歴)
...
| 撮影できなかった（14 枚） | 理由 |
|---|---|
| ~~`S-03`〜`S-05` / `R-03`〜`R-06` / `C-01` / `D-03` / `T-01`~~ | ~~`ANTHROPIC_API_KEY` が無く LLM 実行ができない~~ → ✅ **2026-09-26 に撮影済み**（アプリ用キーをバックエンドにだけ渡し、架空の EC ストア規程を登録して実際に Support / Review / チャンキングを実行。CONFIRM は「拒否」で閉じ、dry-run ON。詳細は README §画面ショット挿入位置） |
| ~~`D-05` / `D-06` / `D-07` / `D-08`~~ | ~~Qdrant 未起動~~ → ✅ **2026-09-26 に撮影済み**（docker デーモンを起動して Qdrant を立て、実データを登録して撮影） |
...
| # | 内容 | 状態 |
|---|---|---|
| 1 | ~~スクリーンショット 10 枚（`S-03`〜`S-05` / `R-03`〜`R-06` / `C-01` / `D-03` / `T-01`）~~ | ✅ **完了**（2026-09-26 撮影。`D-05`〜`D-08` とあわせ全 31 スロットが撮影済み） |
...
| Version | 内容 |
|---|---|
| 2.6 | 残りのスクリーンショット 10 枚を撮影（2026-09-26）。§10 の残タスクはすべて完了 |
| 2.5 | スクリーンショット `D-05`〜`D-08` の 4 枚を撮影（2026-09-26）。残りは `ANTHROPIC_API_KEY` が要る 10 枚だけになった |
